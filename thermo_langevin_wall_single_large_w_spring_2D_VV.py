...
def main(G, phi, psi, Tamp, y0, ReynoldsSett, en, HfillN, StreamN, NondimStiff, sigma_erf, PMM, ic, Tswitch, Theta, seed):
#def main():
    DebugTag = True
    #DebugTag = False
    ...
    if (DebugTag == False):
        ...
        Tamp = sys.argv[4]
        y0 = sys.argv[5]
        ReynoldsSett = (sys.argv[6])
        en = sys.argv[7]
        HfillN = sys.argv[8]
        StreamN = sys.argv[9]
        NondimStiff = sys.argv[10]
        sigma_erf = (sys.argv[11])
        PMM = sys.argv[12]
        ic = sys.argv[13]
        Tswitch = sys.argv[14]
        Theta = sys.argv[15]
        ...
        if int(Tswitch) == 4:
            filename = 'output_2D_LT_VV_T0_' + Tamp + '_G_' + G + '_phi_' + phi + '_psi_' + psi + '_y0_' + y0 + '_ReynoldsSett_' + ReynoldsSett + '_Nc_' + Nstr + '_HfillN_' + HfillN + '_StreamN_' + StreamN + '_sigma_erf_' + sigma_erf + '_NDS_' + NondimStiff + '_PMM_' + PMM + '_en_' + en + '_ic_' + ic + '_TS_' + Tswitch + '_Theta_' + Theta +'_seed_' + seed + '.mat'
        elif int(Tswitch) == 1 or int(Tswitch) == 2 or int(Tswitch) == 3:
            filename = 'output_2D_LT_VV_T0_' + Tamp + '_G_' + G + '_phi_' + phi + '_psi_' + psi + '_y0_' + y0 + '_ReynoldsSett_' + ReynoldsSett + '_Nc_' + Nstr + '_HfillN_' + HfillN + '_StreamN_' + StreamN + '_sigma_erf_' + sigma_erf + '_NDS_' + NondimStiff + '_PMM_' + PMM + '_en_' + en + '_ic_' + ic + '_TS_' + Tswitch + '_seed_' + seed + '.mat'
        elif int(Tswitch) == 0:
            filename = 'output_2D_LT_VV_T0_' + Tamp + '_G_' + G + '_phi_' + phi + '_psi_' + psi + '_y0_' + y0 + '_ReynoldsSett_' + ReynoldsSett + '_Nc_' + Nstr + '_HfillN_' + HfillN + '_StreamN_' + StreamN + '_sigma_erf_' + sigma_erf + '_NDS_' + NondimStiff + '_PMM_' + PMM + '_en_' + en + '_ic_' + ic + '_seed_' + seed + '.mat'
        ...
        N = int(HfillN) * int(StreamN)
        filename = 'temp_' + str(Tamp) +  '_HfillN_' + str(HfillN) + '_NDF_' + str(NondimStiff) + '_y0_' + str(y0) + '_TS_' + str(Tswitch) + '_Theta_' + str(Theta) + '.mat'
    ...
    Nbig, Nsmall, Dbig, Dsmall, rho_s, rho_l, Dn, rho, Vol, m, R_eff, R_hyd, R2n, ind_big, ind_small = (
            MDini.particle_create(N, phi, G, gamma, ic, Dfluc, bimodal_flag))
    ...
    save_reduction = 0.00
    nt, x, y, vx, vy, ax, ay, ax_old, ay_old, CM_big, CM_small, x_data, y_data, vx_data, vy_data, ax_data, ay_data, dy_data_proc, y_data_proc, y_wall_data, ph, T_glb, Tx_glb, Ty_glb, T_big_glb, T_small_glb, Ek_glb, Ekx_glb, Eky_glb, Ek_big_glb, Ek_small_glb, Tx_big_glb, Ty_big_glb, Tx_small_glb, Ty_small_glb, Sxx_data, Sxy_data, Syy_data, area_Voronoi = (
            MDF.langevin_thermostat_w_spring_VV(dt, Nt, m, ind_big, Dn, x, y, vx, vy, ax, ay, ax_old, ay_old, Lx, Ly, N, Nc_org, R_eff, K, B, Tamp, gv, a_thresh, v_thresh, Tswitch, Theta, N_per_coll, save_reduction, y0, plotit, Vor_flag))
    ...
    Sxx_all, Sxy_all, Syy_all, Sxx_big, Sxy_big, Syy_big, Sxx_small, Sxy_small, Syy_small = (
            MDSTR.preproc_2D_ContactStressOrganizeLayer(y_data, Vol, Sxx_data, Sxy_data, Syy_data, bnlmt, bw, ind_big))
    
    P_all, P_big, P_small, tau_all, tau_big, tau_small = MDSTR.preproc_2D_P_tau_from_Sij(Sxx_all, Sxy_all, Syy_all, Sxx_big, Sxy_big, Syy_big, Sxx_small, Sxy_small, Syy_small)
    
    # Get time evolving pressure and shear stress per particle
    P_per_particle, tau_per_particle = MDSTR.preproc_2D_P_tau_from_Sij_per_particle(y_data, Vol, Sxx_data, Sxy_data, Syy_data, ind_big)
...
#(G,phi,psi,Tamp,y0,ReSet,en,HfillN,StreamN,NondimStiff,sigma_erf,PMM,ic,Tswitch,Theta,seed)
...
main(2.00, 0.00, 4.0, 10.0, 5, 2.5, 1.0, 10, 15, 10000, 0.01, 1, 0, 1, 1.0, 5) # uncomment to make it a function
