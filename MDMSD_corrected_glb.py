...
def main(y, dy, Ly, ind_big):
    ...
    for i in range(0, N_dt):   
        y_input = np.transpose(np.asarray(y[i]))
        ...
        if y_input.shape[0] is not 0:
            ...
            for j in range(0, N):
                ...
                if ind_big[j]:
                    ...
                    MSD_l_corrected_all.append(MSD_corrected)
                else:
                    MSD_s_all.append(MSD)
                    MSD_s_corrected_all.append(MSD_corrected)
            # print(np.asarray(MSD_all).shape)
            # print(np.asarray(MSD_corrected_all).shape)
            output.append(np.asarray(MSD_all))
            output_corrected.append(np.asarray(MSD_corrected_all))
            output_l.append(np.asarray(MSD_l_all))
            output_l_corrected.append(np.asarray(MSD_l_corrected_all))
            output_s.append(np.asarray(MSD_s_all))
            output_s_corrected.append(np.asarray(MSD_s_corrected_all))
        else:
            output.append([])
