...
def main():    
    ...
    ##### Drop the top wall #####
    nt, x, y, vx, vy, ax, ay, ax_old, ay_old = (
            drop_wall(dt, Nt, m, Dn, x, y, vx, vy, ax, ay, ax_old, ay_old, Lx, Ly, N, N_org, R_eff, K, B_pour, a_thresh, v_thresh, plotit))
    ...
    ##### Heat the particles #####
    nt, x, y, vx, vy, ax, ay, ax_old, ay_old, CM_big, CM_small, T, Ek, x_data, y_data, vx_data, vy_data, ph = (
            langevin_thermostat(dt, Nt, m, ind_big, Dn, x, y, vx, vy, ax, ay, ax_old, ay_old, Lx, Ly, N, Nc_org, R_eff, K, B, Tamp, gv, a_thresh, v_thresh, plotit))
    ...
    vys_big, vys_small, vys_all, vy_prime = preproc_2D_vs(vy_data, Vol, Vol_sep, y_data, bnlmt, bw)
    vxs_big, vxs_small, vxs_all, vx_prime = preproc_2D_vs(vx_data, Vol, Vol_sep, y_data, bnlmt, bw)
    
    m = m[0:N_org]
    
    # Get time evolving layered kinetic energy and temperature
    Eky_all, Eky_big, Eky_small, Ty_all, Ty_big, Ty_small = preproc_2D_T(vy_prime, m, Vol, Vol_sep, y_data, bnlmt, bw)        
    Ekx_all, Ekx_big, Ekx_small, Tx_all, Tx_big, Tx_small = preproc_2D_T(vx_prime, m, Vol, Vol_sep, y_data, bnlmt, bw)
...
def drop_wall(dt, Nt, m, Dn, x, y, vx, vy, ax, ay, ax_old, ay_old, Lx, Ly, N, Nc_org, R_eff, K, B_pour, a_thresh, v_thresh, plotit):
    g = 0.1
    nt = 0
    flag = False
    while (nt < Nt):
...
@jit
def preproc_2D_vs(vy, m, m_sep, z, bnlmt, proc_bw):
    ...
    for j in range (0, T):
        ...
        vy_small[j][:] = vy_small[j][:] / count_small
        vy_all[j][:] = vy_all[j][:] / count_all
    ...
    return vy_big, vy_small, vy_all, vy_prime

@jit
def preproc_2D_T(vy, m, Vol, Vol_sep, z, bnlmt, proc_bw):
    binsize = bnlmt // proc_bw    
    T = np.size(z, 0) # sampled in time
    N = np.size(z, 1) # length of data (number of particles)
    ...
    for j in range (0, T):
        ...
        for i in range (0, N):
            ind = int(np.ceil(z[j][i] / proc_bw)) - 1
            if (Vol[i] > Vol_sep): # large particle
                Ek_big[j][ind] = Ek_big[j][ind] + 0.5 * m[i] * vy[j][i] * vy[j][i]
                count_big[ind] = count_big[ind] + 1
            if (Vol[i] < Vol_sep): # small particle
                Ek_small[j][ind] = Ek_small[j][ind] + 0.5 * m[i] * vy[j][i] * vy[j][i]
                count_small[ind] = count_small[ind] + 1
        
        Ek_all[j][:] = Ek_big[j][:] + Ek_small[j][:]
...
@jit
def langevin_thermostat(dt, Nt, m, ind_big, Dn, x, y, vx, vy, ax, ay, ax_old, ay_old, Lx, Ly, N, Nc_org, R_eff, K, B, Tamp, gv, a_thresh, v_thresh, plotit):
    ind_big = np.asarray(ind_big, dtype=bool)
    Nbig = sum(ind_big)
    Nsmall = Nc_org - Nbig
    T0 = Tamp
    epsilon = B / m[0:Nc_org] # unit: 1/time
    dt_half = np.sqrt(dt)
    dt2 = dt ** dt
    g = 0.1
    nt = 0
    ...
    while (nt < Nt):
        ...
        x = x + vx * dt
        # y = y + vy * dt
        y[0:Nc_org] = y[0:Nc_org] + vy[0:Nc_org] * dt
        y[-1] = y[-1] + vy[-1] * dt + ay_old[-1] * dt2 / 2
        ...
        sigma = np.sqrt(2 * T0 * epsilon / m[0:Nc_org])
        eta = np.random.randn(Nc_org)
        
        vx[0:Nc_org] = vx[0:Nc_org] + ax[0:Nc_org] * dt - epsilon * dt * vx[0:Nc_org] + dt_half * sigma * eta
        vy[0:Nc_org] = vy[0:Nc_org] + ay[0:Nc_org] * dt - epsilon * dt * vy[0:Nc_org] + dt_half * sigma * eta
        
        vy[-1] = vy[-1] + (ay_old[-1] + ay[-1]) * dt / 2
        ...
        if (nt % saveskip == 0):
            ...
            #vx_mean_mass = (sum(m_CM[ind_big]) / sum(m_CM[0:Nc_org])) * np.mean(vx[ind_big]) + (sum(m_CM[~ind_big]) / sum(m[0:Nc_org])) * np.mean(vx[~ind_big])
            #vy_mean_mass = (sum(m_CM[ind_big]) / sum(m_CM[0:Nc_org])) * np.mean(vy[ind_big]) + (sum(m_CM[~ind_big]) / sum(m[0:Nc_org])) * np.mean(vy[~ind_big])
            Ek_temp = (0.5 * m[0:Nc_org] * ((vx[0:Nc_org] - np.mean(vx[0:Nc_org])) ** 2 + (vy[0:Nc_org] - np.mean(vy[0:Nc_org])) ** 2))
            T_temp = sum(Ek_temp) / Nc_org
            CM_big[count] = np.mean(y_CM[ind_big])
            CM_small[count] = np.mean(y_CM[~ind_big])
            T_list[count] = T_temp
            #Ek_mass_temp = 0.5 * m[0:Nc_org] * ((vx[0:Nc_org] - vx_mean_mass) ** 2 + (vy[0:Nc_org] - vy_mean_mass) ** 2)
            ...
            ph_list[count] = sum(m[0:Nc_org]) / (Lx * y[-1])
    ...
    return nt, x, y, vx, vy, ax, ay, ax_old, ay_old, CM_big, CM_small, T_list, Ek_list, x_data, y_data, vx_data, vy_data, ph_list
...
#main(2.0, 0.5, 4.0, 0.40, 2.5, 1, 24, 1000, 0.01, 0, 2) # uncomment to make it a function
